...
def edge_detection():
    ...
    corners1_edges = cv2.goodFeaturesToTrack(frame1_masked, maxCorners=0, qualityLevel=0.1, minDistance=10)
    corners2_edges = cv2.goodFeaturesToTrack(frame2_masked, maxCorners=0, qualityLevel=0.1, minDistance=10)
    ...
    # optical flow
    nextPts, status, err = cv2.calcOpticalFlowPyrLK(frame1_gray, frame2_gray, corners1_edges, None)
    ...
    for corner in nextPts:
        ...
        x, y = corner.ravel() 
        # Syntax: cv2.circle(image, center, radius, color, thickness)
        cv2.circle(frame2_cp, (x, y), 5, (255, 0, 0), -1)
    
    for corner in nextPts:
        #  Flatten the nested array [[x, y]] into a simple array [x, y]
        x, y = corner.ravel() 
        # Syntax: cv2.circle(image, center, radius, color, thickness)
        cv2.circle(frame1_cp, (x, y), 5, (255, 0, 0), -1)

    # mostrar frame2 detecciones
    cv2.imshow("Optical Flow expected 2", frame2_cp)
    cv2.imshow("Optical Flow expected full", frame1_cp)
